...
async function startPriceCheckCamera(options = {}) {
  ...
  try {
    ...
    try {
      // 720p is optimal for barcodes — higher res = more pixels = slower decode
      state.priceCheckStream = await tryStream({
    ...
    // Apply torch if available (torch = faster decode in poor lighting)
    try {
      const track = state.priceCheckStream?.getVideoTracks()[0];
      const caps = track?.getCapabilities?.();
      if (caps?.torch) {
        await track.applyConstraints({ advanced: [{ torch: true }] });
        state.priceCheckTorchOn = true;
      }
    } catch (_) {}
    ...
    if (Html5Qrcode && scannerEl) {
      try {
        ...
        await state.priceCheckScanner.start(
          ...
          {
            fps: 30,               // Higher fps = faster first detection
            qrbox: { width: 280, height: 120 }, // Smaller box = fewer pixels to decode per frame
            rememberLastUsedCamera: true,
            aspectRatio: 1.7778,
            disableFlip: true,
            formatsToSupport: supportedFormats,
            // CRITICAL: use native BarcodeDetector when available (10-30x faster on Android Chrome)
          ...
          (decodedText) => {
            ...
            const now = Date.now();
            if (code && (code !== state.priceCheckLastCode || now - state.priceCheckLastScanAt > 800)) {
    ...
    if ("BarcodeDetector" in window) {
      ...
      // Wait until video is actually playing frames before starting decode loop
      const waitForFrames = () => {
        const v = priceCheckVideoEl();
        if (v && v.readyState >= 2 && v.videoWidth > 0) {
          state.priceCheckScanInterval = null; // clear so scanPriceCheckFrame can start fresh
          scanPriceCheckFrame();
        } else {
...
function stopPriceCheckCamera() {
  ...
  if (state.priceCheckScanInterval) {
    clearInterval(state.priceCheckScanInterval);
    state.priceCheckScanInterval = null;
  }
  // Turn off torch if it was on (fire-and-forget, no await needed)
  try {
    if (state.priceCheckTorchOn) {
      const track = state.priceCheckStream?.getVideoTracks()[0];
...
async function scanPriceCheckFrame() {
  // Use setInterval instead of RAF for more consistent 25fps scan rate
  if (state.priceCheckScanInterval) return; // already running
  state.priceCheckScanInterval = setInterval(async () => {
    const videoEl = priceCheckVideoEl();
    if (!state.priceCheckDetector || !videoEl || videoEl.readyState < 2) return;
    try {
      // Decode from a small canvas crop (center 60% of frame) — far fewer pixels
      const canvas = document.createElement("canvas");
      const vw = videoEl.videoWidth || 640;
      const vh = videoEl.videoHeight || 480;
      const cx = Math.floor(vw * 0.1);  // 10% inset
      const cy = Math.floor(vh * 0.25); // 25% inset
      const cw = Math.floor(vw * 0.8);  // 80% width
      const ch = Math.floor(vh * 0.5);  // 50% height
      canvas.width  = cw;
      canvas.height = ch;
